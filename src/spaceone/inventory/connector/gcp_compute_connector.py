...
class GcpComputeConnector(BaseConnector):
    ...
    def verify(self, options, credentials):
        ...
        self._setConnect(credentials)
        return "ACTIVE"
        # return self.conf

    def _setConnect(self, cred):
        """
        cred(dict)
            - type: ..
            - project_id: ...
            ...
            - ...
        """
        self.cred = google.oauth2.service_account.Credentials.from_service_account_info(cred)
        self.project_id = cred['project_id']
        self.zone = cred['region']
    ...
    def _composite_disk_data(self, instance, zone):
        ...
        for disk in instance['disks']:
            ...
            if "sourceImage" in disktype_dic.keys():
                ...
                ret_dic['data']["os"]["os_details"] = sourceimagetype_dic["description"] if "description" in sourceimagetype_dic else ""
                ...
                if "targetTags" in firewall.keys():
                    ...
                    for firewall_target_serviceaccount in firewall_target_serviceaccount_list:
                        ...
                        if firewall_target_serviceaccount == svcs:
                            ret_security_group_rules_list.extend(self.make_firewall_dict("targetServiceAccounts",firewall))
                            ret_security_group_rules_name.append(firewall["name"])
...
# ALLOW or Deny 루프 한번 돌아야 하니 나중에
    ...
    def _parse_subnetwork_type(self, project, zone, subnetwork):
        ...
        request = self.client.subnetworks().get(project=self.project_id, region=items[-3], subnetwork=items[-1]).execute()
